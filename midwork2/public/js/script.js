$(document).ready(function () {
    ...
    $("#userupdate").on('click','.btn-cancel',updateCancel);
    // $("#uploadimg").on('click','.submitimg',updateImg);
    // $("#loginenter").on('click','.login-submit',loginUser);
    // $("#loginenter").on('click','.register-submit',registerUser);
    // $("#registerenter").on('click','.register-stored',registerSumit);
    $("a").click(function(event){
    $("a").attr("target","_blank");});
    $(".sumitimg").click(function(event){
    $(".imgput").append("<div><img></div>");});
    // function updateImg(event){
    //     var newImg='newImage' + '-' + Today.getFullYear()+'-'+(Today.getMonth()+1)+'-'+Today.getDate()+'-'+Today.getHours()+'-'+Today.getMinutes()+'-'+Today.getSeconds();
    //         $.ajax({
    //             type:'POST',
    //             data: newImg,
    //             url:'update',
    ...
    //             var html=new EJS({url:'views/uploadimg.ejs'}).render(res);
    //             $(".wrapper").append(html);
